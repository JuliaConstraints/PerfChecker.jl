...
function _series_catalog(bundle::RunBundle)
    ...
    for item in series
        ...
        for (kind, label) in kinds
            push!(entries, merge(copy(common), Dict{String, Any}(
...
function _allocation_observations(bundle::RunBundle)
    return [observation for observation in bundle.observations
            if get(observation, "measurement_definition", "") in
               ("julia.alloc.bytes/line-tracking-v1",
                "julia.alloc.bytes/profile-allocs-v1") &&
               get(get(observation, "attributes", Dict()), "source_file", nothing) !== nothing]
end
...
function _allocation_catalog(bundle::RunBundle)
    ...
    for ((package, feature, case_id), common) in sort!(collect(identities); by = first)
        ...
        if any(observation -> get(get(observation, "attributes", Dict()),
                "stack", nothing) isa AbstractVector, observations)
            push!(entries, merge(copy(common), Dict{String, Any}(
...
function _profile_catalog(bundle::RunBundle)
    ...
    for observation in bundle.observations
        get(observation, "measurement_definition", "") ==
            "julia.cpu.samples/profile-v1" || continue
    ...
    return [merge(copy(common), Dict{String, Any}(
                "id" => _plot_id(:cpu_flamegraph, case_id),
                "kind" => "cpu_flamegraph",
                "title" => "$package · $feature · CPU flame graph",
                "label" => "CPU flame graph", "metric" => "julia.cpu.samples",
                "unit" => "samples"))
            for ((package, feature, case_id), common) in sort!(collect(identities); by = first)]
end
...
function _tradeoff_catalog(bundle::RunBundle)
    ...
    for ((package, feature, comparison_key), metrics) in sort!(collect(grouped); by = first)
        all(haskey(metrics, metric) for metric in ("julia.wall.time", "julia.alloc.bytes")) ||
            continue
        push!(entries, Dict{String, Any}(
            "id" => _plot_id(:time_allocation_tradeoff, package, feature, comparison_key),
...
function plot_catalog(bundle::RunBundle)
    ...
    sort!(plots; by = item -> (String(get(item, "package", "")),
...
function _series_observations(bundle::RunBundle, series)
    return [observation for observation in bundle.observations if
...
function _delta_records(bundle, series)
    ...
    return [copy(record) for record in comparison.records
            if get(record, "series_id", "") == series["series_id"]]
end

function _allocation_records(bundle, case_id::String)
    return [observation for observation in _allocation_observations(bundle)
...
function _allocation_file_records(bundle, case_id)
    ...
    sort!(versions; by = item -> _version_point_key(Dict("version" => item,
...
function _allocation_line_records(bundle, case_id; version = nothing, top::Integer = 40)
    ...
    versions = unique!(String[first(key) for key in keys(grouped)])
    sort!(versions; by = _version_point_key ∘ (item -> Dict("version" => item,
        "target_kind" => startswith(item, "dev@") ? "dev" : "release")))
    selected = version === nothing ? (isempty(versions) ? "" : last(versions)) : String(version)
...
function _allocation_heatmap_records(bundle, case_id; top::Integer = 40)
    ...
    sort!(versions; by = item -> _version_point_key(Dict("version" => item,
        "target_kind" => startswith(item, "dev@") ? "dev" : "release")))
    labels = first.(first(sort!(collect(totals); by = item -> -last(item)), min(length(totals), max(Int(top), 1))))
...
function _flame_records(observations; version = nothing, top::Integer = 40)
    ...
    sort!(versions; by = item -> _version_point_key(Dict("version" => item,
        "target_kind" => startswith(item, "dev@") ? "dev" : "release")))
    selected = version === nothing ? (isempty(versions) ? "" : last(versions)) : String(version)
    grouped = Dict{Tuple{Vararg{String}}, Float64}()
    for observation in observations
        ...
        grouped[labels] = get(grouped, labels, 0.0) + Float64(observation["value"])
    end
    stacks = sort!(collect(grouped); by = item -> -last(item))
    ...
    function visit(children, depth, start, path)
        ...
        for (label, child) in ordered
            ...
            push!(records, Dict{String, Any}(
                ...
                "percentage" => total == 0 ? 0.0 : 100 * Float64(child["value"]) / total))
...
function _tradeoff_records(bundle, entry)
    matching = [series for series in suite_version_series(bundle)
...
function performance_plot(bundle::RunBundle, id::AbstractString; version = nothing,
    ...
    if kind in (:version_series, :distribution, :version_delta)
        ...
        encoding = kind === :version_delta ?
            Dict("x" => "candidate_version", "y" => "relative_delta", "color" => "status") :
    ...
    elseif kind === :allocation_flamegraph
        ...
        encoding = Dict("x" => "percentage", "y" => "depth", "color" => "label")
    elseif kind === :cpu_flamegraph
        observations = [observation for observation in bundle.observations
...
@testitem "Performance plot grammar" tags=[:unit, :plots] begin
    ...
    observations = [Dict{String, Any}(
        ...
        "metric" => "julia.wall.time", "measurement_definition" =>
            "julia.wall.time/benchmarktools-v1", "value" => value, "unit" => "s",
        "attributes" => Dict("package" => "Demo", "feature" => "parse",
            "version" => version, "target_kind" => kind))
        for (version, kind, value) in (("1.0.0", "release", 1.0),
                                      ("1.1.0", "release", 0.8),
                                      ("dev@1.2.0", "dev", 0.7))]
    bundle = RunBundle(Dict{String, Any}("run_id" => "plot-run", "suite" => "demo",
        "state" => "complete"), definitions, observations,
...
@testitem "Allocation plot grammar" tags=[:unit, :plots, :allocations] begin
    ...
    for (version, kind) in (("1.0.0", "release"), ("dev@1.1.0", "dev"))
        for (file, line, bytes) in (("src/parse.jl", 10, 100.0),
            ...
            push!(observations, Dict{String, Any}(
                ...
                "comparison_key" =>
                    "parse/v1::julia.alloc.bytes/line-tracking-v1",
                "metric" => "julia.alloc.bytes", "measurement_definition" =>
                    "julia.alloc.bytes/line-tracking-v1", "value" => bytes,
    ...
    bundle = RunBundle(Dict{String, Any}("run_id" => "allocation-plot-run",
        "suite" => "demo", "state" => "complete"), [definition], observations,
        Dict{String, Any}[], Dict{String, Any}[])
    catalog = plot_catalog(bundle)
...
@testitem "CPU flame graph grammar" tags=[:unit, :plots, :profile, :flamegraph] begin
    ...
    observations = [Dict{String, Any}(
        ...
        "metric" => "julia.cpu.samples", "measurement_definition" =>
            "julia.cpu.samples/profile-v1", "value" => samples, "unit" => "1",
        "attributes" => Dict{String, Any}(
            "package" => "Demo", "feature" => "parse_profile",
            "version" => "dev@1.1.0", "target_kind" => "dev",
            "source_file" => "src/parse.jl", "source_line" => 10,
            "stack" => stack))
        for (samples, stack) in ((8, ["parse", "tokenize"]),
                                 (2, ["parse", "normalize"]))]
    bundle = RunBundle(Dict{String, Any}("run_id" => "profile-plot-run",
        "suite" => "demo", "state" => "complete"), [definition], observations,
        Dict{String, Any}[], Dict{String, Any}[])
    entry = only(filter(item -> item["kind"] == "cpu_flamegraph",
        plot_catalog(bundle)))
    plot = performance_plot(bundle, entry["id"])
    @test plot.options["total"] == 10

...
function formatValue(value, key = '', unit = '') {
    if (!finite(value))
        return value === null || value === undefined ? '—' : String(value);
    if (unit === 'ns' || key.endsWith('_time'))
        return formatSeconds(value / 1e9);
    if (unit === 's')
        return formatSeconds(value);
    if (unit === 'By' || key.includes('memory') || key.includes('bytes'))
        return formatBytes(value);
    if (unit === '%')
        return `${compact(value)}%`;
    return new Intl.NumberFormat(undefined, { maximumFractionDigits: 2 }).format(value);
}
function sparkline(series, highlightedVersions, id) {
    const points = series.points.filter(point => finite(point.median));
    if (!points.length)
        return '<div class="empty-chart">No numeric samples</div>';
    ...
    const y = (value) => maximum === minimum ? height / 2 :
        height - padding - (value - minimum) * (height - padding * 2) / (maximum - minimum);
    const coordinates = points.map((point, index) => `${x(index)},${y(point.median)}`).join(' ');
    const circles = points.map((point, index) => {
        ...
        return `<circle class="point hover-value${selected}" tabindex="0" data-detail="${html(label)}" data-target="${html(id)}" cx="${x(index)}" cy="${y(point.median)}" r="${selected ? 5 : 3}"><title>${html(label)}</title></circle>`;
    }).join('');
...
function distributionPlot(observations, id) {
    ...
    const height = 100;
    const padding = 18;
    const x = (value) => maximum === minimum ? width / 2 : padding + (value - minimum) * (width - padding * 2) / (maximum - minimum);
...
function allocationPlot(observations, id) {
    ...
    const wedges = entries.map(([label, value], index) => {
        ...
        const x2 = 100 + 82 * Math.cos(next);
        const y2 = 100 + 82 * Math.sin(next);
...
function flameGraph(observations, id) {
    if (!observations.length)
        return '';
    const root = { name: 'root', value: 0, children: new Map(), dynamic: false,
        unstable: false, gc: false, inferredTypes: new Set() };
    for (const observation of observations) {
        if (!finite(observation.value) || observation.value <= 0)
            continue;
        ...
        for (let index = 0; index < stack.length; index += 1) {
            ...
            if (inferred)
                node.inferredTypes.add(inferred);
            parent = node;
        }
    }
    if (!root.value)
    ...
    const width = 1000;
    const rowHeight = 25;
    const visit = (node, left, span, depth) => {
        if (rendered >= 1800 || span < 0.12)
            return;
...
class PerfNode {
    ...
    runs;
    parent;
    constructor(kind, label, runs, parent) {
        this.kind = kind;
        this.label = label;
        this.runs = runs;
        this.parent = parent;
...
class PlanTree {
    dragMimeTypes = ['application/vnd.code.tree.perfchecker.runs'];
    dropMimeTypes = this.dragMimeTypes;
    ...
    getTreeItem(node) {
        const item = new vscode.TreeItem(node.label, node.kind === 'run' ? vscode.TreeItemCollapsibleState.None : vscode.TreeItemCollapsibleState.Collapsed);
    ...
    getChildren(node) {
        ...
        if (!node) {
            return [...new Set(runs.map(run => run.package))].map(name => new PerfNode('package', name, runs.filter(run => run.package === name)));
        }
        if (node.kind === 'package') {
            return [...new Set(runs.map(model_1.logicalFeature))].map(name => new PerfNode('feature', name, runs.filter(run => (0, model_1.logicalFeature)(run) === name), node));
        }
        if (node.kind === 'feature') {
            return [...new Set(runs.map(run => run.backend))].map(backend => new PerfNode('check', checkLabel(backend), runs.filter(run => run.backend === backend), node));
        }
        if (node.kind === 'check') {
    ...
    async handleDrop(target, data) {
        ...
        const ids = item ? JSON.parse(await item.asString()) : [];
        const targetId = target?.runs[0]?.id;
        if (!targetId)
            return;
        for (const id of ids)
            this.order = (0, model_1.moveRun)(this.order, id, targetId);
        this.changed.fire(undefined);
    }
}
class Controller {
    ...
    tree;
    tests;
    output = vscode.window.createOutputChannel('PerfChecker');
    designer;
    resultsPanel;
    plan;
    uiConfiguration;
    constructor(context, tree, tests) {
        this.context = context;
        this.tree = tree;
        this.tests = tests;
    }
    reportError(error) { this.output.appendLine(String(error)); }
    showLog() { this.output.show(true); }
    root() {
        const folder = vscode.workspace.workspaceFolders?.[0];
        if (!folder)
            throw new Error('Open a package workspace first.');
        return folder.uri.fsPath;
    }
    setting(name) {
        return vscode.workspace.getConfiguration('perfchecker').get(name);
    }
    gitTargets() {
        return vscode.workspace.getConfiguration('perfchecker').get('gitTargets', []);
    }
    effectiveGitTargets() {
        const inspected = vscode.workspace.getConfiguration('perfchecker').inspect('gitTargets');
    ...
    candidateArguments(targets = this.effectiveGitTargets()) {
        return targets.map(target => `--candidate=${JSON.stringify(target)}`);
    }
    async git(args, cwd = this.root()) {
        return await new Promise((resolve, reject) => {
            const child = (0, node_child_process_1.spawn)('git', args, { cwd, windowsHide: true });
            let stdout = '';
            let stderr = '';
    ...
    async discoverGitReferences(packageName, sourceInput = '') {
        ...
        const seen = new Set();
        const add = (option) => {
            if (!option.revision || option.revision === 'HEAD' || seen.has(option.revision))
                return;
            seen.add(option.revision);
            options.push(option);
        };
        if (/^(?:https?|ssh):\/\//i.test(source) || /^git@[^:]+:/i.test(source)) {
            ...
            for (const line of refs.split(/\r?\n/)) {
                ...
                if (branch)
                    add({ kind: 'branch', label: branch[1], revision: branch[1], detail: sha.slice(0, 12) });
                if (tag)
                    add({ kind: 'tag', label: tag[1], revision: tag[1], detail: sha.slice(0, 12) });
            }
        ...
        const stat = await node_fs_2.promises.stat(local);
        ...
        for (const line of commits.split(/\r?\n/)) {
            ...
            if (revision)
                add({ kind: 'commit', label: short, revision,
                    detail: subjectParts.join('\t') });
        }
        return { repository, source: requestedSource, options };
    }
    comparisonPolicies() {
        return vscode.workspace.getConfiguration('perfchecker').get('comparisonPolicies', []);
    }
    effectiveComparisonPolicies() {
        const inspected = vscode.workspace.getConfiguration('perfchecker').inspect('comparisonPolicies');
        const explicitlyConfigured = inspected?.workspaceFolderValue !== undefined ||
            inspected?.workspaceValue !== undefined || inspected?.globalValue !== undefined;
        return explicitlyConfigured ? this.comparisonPolicies() :
            (this.uiConfiguration?.comparisons ?? this.comparisonPolicies());
    }
    comparisonArguments(policies = this.effectiveComparisonPolicies()) {
        return policies.map(policy => `--comparison=${JSON.stringify(policy)}`);
    }
    absolute(setting) {
        return path.resolve(this.root(), this.setting(setting));
    }
    async invoke(command, args, progress, emitted) {
        ...
        return await new Promise((resolve, reject) => {
            const child = (0, node_child_process_1.spawn)(executable, juliaArgs, { cwd: this.root(), windowsHide: true });
            child.stdout.setEncoding('utf8');
            child.stderr.setEncoding('utf8');
            let pending = '';
            const publish = (line) => {
                ...
                if (line.startsWith('PERFCHECKER_PROGRESS ')) {
                    try {
                        progress?.(JSON.parse(line.slice(21)));
                        return;
                    }
                    catch { /* preserve malformed progress as output */ }
                }
            ...
            child.stdout.on('end', () => { if (pending)
    ...
    async refresh() {
        await vscode.window.withProgress({ location: vscode.ProgressLocation.Window, title: 'PerfChecker: planning' }, async () => {
            await node_fs_2.promises.mkdir(this.context.globalStorageUri.fsPath, { recursive: true });
            try {
                this.uiConfiguration = JSON.parse(await node_fs_2.promises.readFile(this.absolute('uiConfiguration'), 'utf8'));
            }
            catch {
            ...
            if (code !== 0)
                throw new Error(`PerfChecker plan failed with code ${code}.`);
            const plan = JSON.parse(await node_fs_2.promises.readFile(output, 'utf8'));
            if (plan.schema_version !== 'perfchecker-suite-plan/1')
                throw new Error('Unsupported suite plan.');
            this.plan = plan;
            this.tree.refresh(plan);
            try {
                if (!this.uiConfiguration)
                    throw new Error('No saved UI configuration');
                const saved = (this.uiConfiguration.selection?.run_ids ?? []);
    ...
    syncTests() {
        ...
        for (const packageName of [...new Set(this.plan.runs.map(run => run.package))]) {
            ...
            for (const featureName of [...new Set(packageRuns.map(model_1.logicalFeature))]) {
                ...
                const featureRuns = packageRuns.filter(item => (0, model_1.logicalFeature)(item) === featureName);
    ...
    collectTestIds(items) {
        ...
        if (items)
            ...
            this.tests.items.forEach(visit);
        return collected;
    }
    async runTests(request) {
        ...
        ids.forEach(id => { const item = items.get(id); if (item)
            run.enqueued(item); });
        ...
        try {
            ...
            const code = await this.invoke('run', args, value => {
                const current = value.current_run?.id;
            ...
            for (const id of ids) {
                ...
                if (planned?.status !== 'ready')
                    ...
                    run.failed(item, new vscode.TestMessage(`Inspect the PerfChecker report; the suite exited with code ${code}.`));
            }
        }
        catch (error) {
            for (const id of ids) {
    ...
    async initialize() {
        ...
        if (code !== 0)
            throw new Error(`PerfChecker initialization failed with code ${code}.`);
        await this.refresh();
    }
    ids(node) {
        return node ? node.runs.map(run => run.id) : [...this.tree.selected];
    }
    async run(node, explicitIds, revealOutput = false) {
        if (!this.plan)
            await this.refresh();
        ...
        await vscode.window.withProgress({
            ...
            if (code !== 0)
                throw new Error(`PerfChecker run failed with code ${code}.`);
        });
        void vscode.window.showInformationMessage('PerfChecker run completed.');
    }
    async open(nodeOrRun) {
    ...
    async readReport(name) {
        try {
            return JSON.parse(await node_fs_2.promises.readFile(path.join(this.absolute('reports'), name), 'utf8'));
        }
        catch (error) {
    ...
    async bundleDirectory(runId) {
        const root = path.join(this.absolute('reports'), 'bundles');
        if (runId) {
            const expected = path.join(root, `run-${runId}`);
            try {
                if ((await node_fs_2.promises.stat(expected)).isDirectory())
                    return expected;
            }
            catch { /* fall back to newest */ }
        }
        try {
            const entries = (await node_fs_2.promises.readdir(root, { withFileTypes: true })).filter(entry => entry.isDirectory());
            const candidates = await Promise.all(entries.map(async (entry) => ({
                path: path.join(root, entry.name), modified: (await node_fs_2.promises.stat(path.join(root, entry.name))).mtimeMs,
            })));
    ...
    async reportObservations(runs, runId) {
        const bundle = await this.bundleDirectory(runId);
        if (!bundle)
            return [];
        const source = path.join(bundle, 'observations.jsonl');
        try {
            await node_fs_2.promises.access(source);
        }
        catch {
            return [];
        }
        const result = [];
        const stream = (0, node_fs_1.createReadStream)(source, { encoding: 'utf8' });
        const lines = readline.createInterface({ input: stream, crlfDelay: Infinity });
        for await (const line of lines) {
            if (!line.includes('"record_type":"observation"'))
                continue;
            try {
    ...
    async openReportFile(name) {
        ...
        try {
            await node_fs_2.promises.access(uri.fsPath);
            if (name.endsWith('.md'))
            ...
            void vscode.window.showWarningMessage(`PerfChecker has not produced ${name} yet.`);
        }
    }
    resultPage(title, runs, suite, versionFile, comparisonFile, observations = []) {
        const nonce = this.nonce();
        const outputs = (0, model_1.outputsForRuns)(suite?.runs ?? [], runs);
        const selectedVersions = new Set(runs.map(run => run.version));
        const series = (0, model_1.seriesForRuns)(versionFile?.series ?? [], runs);
        let comparisons = (0, model_1.comparisonsForRuns)(comparisonFile?.records ?? [], runs);
        if (runs.length === 1)
            comparisons = comparisons.filter(record => selectedVersions.has(record.baseline_version) || selectedVersions.has(record.candidate_version));
        const definitionBackend = (definition) => definition.includes('chairmarks') ? 'chairmark' :
        ...
        const filterAttributes = (kind, packageName, workload, backend, version, status, search) => `data-result-item data-kind="${html(kind)}" data-package="${html(packageName)}" ` +
        ...
        const countCards = [...counts.entries()].map(([status, count]) => `<div class="stat ${html(status)}"><strong>${count}</strong><span>${html(status)}</span></div>`).join('');
        const runCards = outputs.map(output => {
            ...
            const workload = output.workload ?? (0, model_1.logicalFeature)({ feature: output.feature, backend: runs.find(run => run.feature === output.feature)?.backend ?? 'benchmark' });
            ...
            const attributes = filterAttributes('run', output.package, workload, backend, output.version, output.status, `${output.package} ${workload} ${checkLabel(backend)} ${output.version} ${output.status}`);
        ...
        const seriesCards = series.map((item, index) => {
            ...
            const workload = item.workload ?? (0, model_1.logicalFeature)({ feature: item.feature, backend });
            const attributes = filterAttributes('series', item.package, workload, backend, item.points.map(point => point.version).join(' '), '', `${item.package} ${workload} ${collector} ${item.metric}`);
        ...
        const comparisonRows = visibleComparisons.map(record => {
            ...
            const attributes = filterAttributes('comparison', record.package, workload, backend, `${record.baseline_version} ${record.candidate_version}`, record.status, `${record.package} ${workload} ${record.metric} ${record.baseline_version} ${record.candidate_version} ${record.status}`);
        ...
        const flames = [...groups.entries()].slice(0, 24).map(([key, group], index) => {
            ...
            const workload = group[0].attributes.workload ?? (0, model_1.logicalFeature)({ feature: group[0].attributes.feature ?? caseId, backend });
        ...
        const optionList = (values) => [...new Set(values.filter(Boolean))].sort()
    ...
    async openOutput(nodeOrRun, explicitRuns) {
        if (!this.plan)
            await this.refresh();
        ...
        const [suite, versionFile, comparisonFile] = await Promise.all([
            this.readReport('suite-result.json'),
            this.readReport('version-series.json'),
            this.readReport('version-comparison.json'),
        ]);
        const observations = await this.reportObservations(runs, versionFile?.run_id);
        if (!this.resultsPanel) {
            this.resultsPanel = vscode.window.createWebviewPanel('perfchecker.output', 'PerfChecker output', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
            ...
            this.resultsPanel.webview.onDidReceiveMessage(async (message) => {
                if (message.type === 'report')
                    await this.openReportFile(String(message.name));
            });
        }
        else
    ...
    nonce() { return Math.random().toString(36).slice(2) + Date.now().toString(36); }
    async openDesigner() {
        if (!this.plan)
            await this.refresh();
        if (this.designer) {
            this.designer.reveal();
            this.postPlan();
            return;
        }
        this.designer = vscode.window.createWebviewPanel('perfchecker.designer', 'PerfChecker suite', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
        const nonce = this.nonce();
      ...
      <aside><section class="aside-panel"><h2>Comparison targets</h2><p class="hint">Choose a discovered branch, tag, or recent commit. You can also paste a GitHub/GitLab URL or a reference such as <code>owner/repository@branch</code>.</p><div id="target-list"></div><label>Package<select id="target-package"></select></label><label>Discovered Git reference<select id="target-reference"><option value="">Loading references…</option></select></label><div class="target-scan"><small id="target-source-status">Looking for the package repository…</small><button id="refresh-targets" title="Scan Git references again">Refresh</button></div><label>Or paste a reference<input id="target-revision" placeholder="Branch, tag, commit, or Git URL"></label><label>Display label (optional)<input id="target-label" placeholder="Filled from the selected reference"></label><details><summary>Advanced target options</summary><label>Git source override<input id="target-source" placeholder="Use the package source"></label><label>Workload compatibility<select id="target-compatibility"><option value="">Automatic</option></select></label></details><p id="target-error" class="form-error" hidden></p><button id="add-target">Add comparison target</button></section><section class="aside-panel"><h2>Comparison matrix</h2><p class="hint">One checked reference is an exact comparison. Several references form an aggregated reference group.</p><div id="comparison-list"></div><label>Package<select id="comparison-package"></select></label><label>Feature<select id="comparison-feature"></select></label><label>Reference aggregation<select id="comparison-aggregation"><option value="median">Median</option><option value="mean">Mean</option><option value="minimum">Minimum</option><option value="maximum">Maximum</option></select></label><h3>Reference targets</h3><div id="baseline-targets" class="target-options"></div><h3>Candidate targets</h3><div id="candidate-targets" class="target-options"></div><button id="add-comparison">Add comparison</button></section><section class="aside-panel"><h2>Documentation block</h2><label>Identifier<input id="doc-id" value="performance"></label><label>Title<input id="doc-title" value="Performance"></label><label>Interactive URL<input id="doc-url" placeholder="https://…"></label><fieldset><legend>Views</legend><label><input type="checkbox" name="view" value="summary" checked> Summary</label><label><input type="checkbox" name="view" value="comparison" checked> Comparisons</label><label><input type="checkbox" name="view" value="plots" checked> Plots</label><label><input type="checkbox" name="view" value="observations"> Observations</label><label><input type="checkbox" name="view" value="diagnostics"> Diagnostics</label><label><input type="checkbox" name="view" value="artifacts"> Artifacts</label></fieldset><p class="hint">The saved JSON is consumed by VS Code, Oxygen-compatible tooling and Documenter via <code>read_document_blocks</code>.</p></section></aside></main><script nonce="${nonce}" src="${script}"></script></body></html>`;
        ...
        this.designer.webview.onDidReceiveMessage(async (message) => {
            if (message.type === 'open')
                await this.open(message.run);
            if (message.type === 'run')
                await this.run(undefined, message.ids, Boolean(message.reveal));
            if (message.type === 'output')
                await this.openOutput(message.run, message.runs);
            if (message.type === 'refresh')
                await this.refresh();
            if (message.type === 'save')
                await this.saveConfiguration(message.configuration);
            if (message.type === 'targets')
                await this.updateGitTargets(message.targets);
            ...
            if (message.type === 'addTarget') {
                try {
                    await this.addGitTarget(message.target);
                }
                catch (error) {
                    void this.designer?.webview.postMessage({ type: 'targetError', error: String(error) });
                }
            }
            if (message.type === 'comparisons')
                await this.updateComparisonPolicies(message.comparisons);
        });
        this.postPlan();
    }
    postPlan() {
        if (this.plan && this.designer)
            void this.designer.webview.postMessage({ type: 'plan', plan: this.plan,
                configuration: this.uiConfiguration, targets: this.effectiveGitTargets(),
                comparisons: this.effectiveComparisonPolicies() });
    }
    async updateGitTargets(targets) {
        ...
        if (new Set(identities).size !== identities.length)
            throw new Error('Git target labels must be unique per package.');
        await vscode.workspace.getConfiguration('perfchecker').update('gitTargets', normalized, vscode.ConfigurationTarget.WorkspaceFolder);
        await this.refresh();
    }
    async addGitTarget(input) {
        const parsed = (0, model_1.parseGitReference)(String(input?.reference ?? ''), String(input?.source ?? ''));
        const label = String(input?.label ?? '').trim() || parsed.suggestedLabel;
        const compatibility = String(input?.compatibility_version ?? '').trim();
        const target = { package: String(input?.package ?? '').trim(), label,
    ...
    async updateComparisonPolicies(policies) {
        ...
        await vscode.workspace.getConfiguration('perfchecker').update('comparisonPolicies', normalized, vscode.ConfigurationTarget.WorkspaceFolder);
        await this.refresh();
    }
    async saveConfiguration(configuration) {
        if (!configuration)
            throw new Error('Open the visual suite editor first.');
        const destination = this.absolute('uiConfiguration');
        await node_fs_2.promises.mkdir(path.dirname(destination), { recursive: true });
        await node_fs_2.promises.writeFile(destination, JSON.stringify(configuration, null, 2) + '\n', 'utf8');
...
function activate(context) {
    ...
    tests.createRunProfile('Run', vscode.TestRunProfileKind.Run, request => controller.runTests(request), true);
    ...
    context.subscriptions.push(view, tests, vscode.commands.registerCommand('perfchecker.refresh', () => controller.refresh()), vscode.commands.registerCommand('perfchecker.initialize', () => controller.initialize()), vscode.commands.registerCommand('perfchecker.runAll', () => controller.run(undefined, tree.plan?.runs.map(run => run.id))), vscode.commands.registerCommand('perfchecker.runNode', (node) => controller.run(node)), vscode.commands.registerCommand('perfchecker.openEntrypoint', (node) => controller.open(node)), vscode.commands.registerCommand('perfchecker.openOutput', (node) => controller.openOutput(node)), vscode.commands.registerCommand('perfchecker.showLog', () => controller.showLog()), vscode.commands.registerCommand('perfchecker.openDesigner', () => controller.openDesigner()), vscode.commands.registerCommand('perfchecker.saveConfiguration', () => controller.saveConfiguration()), view.onDidChangeCheckboxState(event => {
        for (const [node, state] of event.items) {
            for (const run of node.runs)
                state === vscode.TreeItemCheckboxState.Checked ? tree.selected.add(run.id) : tree.selected.delete(run.id);
        }
    }));
